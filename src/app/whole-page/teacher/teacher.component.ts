...
export class TeacherComponent implements OnInit {
    ...
    constructor(private formBuilder: FormBuilder,
        ...
        this.newProjForm = formBuilder.group({
            ...
            scPrId: new FormControl('', Validators.required),
  ...
  ngOnInit(): void {
      ...
      this.getScPriority(0, this.lang);
  ...
  // }
    ...
    getScPriority(id, lang): void {
      this._api.getScPriority(id, lang).subscribe(
          res => {
              console.log(res);
              for (let i = 0; i < res.length; i++) {
                  const temp = {
                      value: res[i].prId,
                      viewValue: res[i].prName
                  };
                  this.dividers[i] = temp;
              }
          }, err => console.log(err)
      );
    }

    selectScSubPriority(id): void {
        this._api.getScPriority(id, this.translateService.currentLang).subscribe(
            res => {
                console.log(res);
                for (let i = 0; i < res.length; i++) {
                    const temp = {
    ...
    selectScSubSubPriority(id): void {
        this._api.getScPriority(id, this.translateService.currentLang).subscribe(
            res => {
                console.log(res);
                for (let i = 0; i < res.length; i++) {
