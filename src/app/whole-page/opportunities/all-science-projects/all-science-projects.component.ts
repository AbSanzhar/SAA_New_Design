    ...
    this._api.getAllScienceProjects().subscribe(
        res => {
          ...
          for(let i = 0; i < res.length; i++) {
              const agrDay = new Date(res[i].scAgrDate).getDate() < 10 ? '0' + new Date(res[i].scAgrDate).getDate() : new Date(res[i].scAgrDate).getDate();
              const agrMonth = (new Date(res[i].scAgrDate).getMonth() + 1) < 10 ? '0' + (new Date(res[i].scAgrDate).getMonth() + 1) : (new Date(res[i].scAgrDate).getMonth() + 1);
              const agrYear = new Date(res[i].scAgrDate).getFullYear();
              this.allScienceProjecrs[i].scAgrDate = agrDay + '/' + agrMonth + '/' + agrYear;

              const stDay = new Date(res[i].scStDate).getDate() < 10 ? '0' + new Date(res[i].scStDate).getDate() : new Date(res[i].scStDate).getDate();
              const stMonth = (new Date(res[i].scStDate).getMonth() + 1) < 10 ? '0' + (new Date(res[i].scStDate).getMonth() + 1) : (new Date(res[i].scStDate).getMonth() + 1);
              const stYear = new Date(res[i].scStDate).getFullYear();
              this.allScienceProjecrs[i].scStDate = stDay + '/' + stMonth + '/' + stYear;

              const endDay = new Date(res[i].scEndDate).getDate() < 10 ? '0' + new Date(res[i].scEndDate).getDate() : new Date(res[i].scEndDate).getDate();
              const endMonth = (new Date(res[i].scEndDate).getMonth() + 1) < 10 ? '0' + (new Date(res[i].scEndDate).getMonth() + 1) : (new Date(res[i].scEndDate).getMonth() + 1);
