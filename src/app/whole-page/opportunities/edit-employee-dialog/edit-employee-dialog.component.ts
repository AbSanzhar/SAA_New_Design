...
export class EditEmployeeDialogComponent implements OnInit {
  formOptions: FormGroup = new FormGroup({
    option1: new FormControl(this.data.roles[0].roleName === 'professor' ? true : false),
    option2: new FormControl(this.data.roles[1].roleName === 'department of science' ? true : false),
    option3: new FormControl(this.data.roles[2].roleName === 'secretary of the council' ? true : false)
  });
  ...
  saveChange() {
    let newForm = new FormGroup({
      ...
      middleName: new FormControl(this.data.patronymic),
